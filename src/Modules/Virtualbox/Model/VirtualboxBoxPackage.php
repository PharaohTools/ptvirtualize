...
class VirtualboxBoxPackage extends BaseVirtualboxAllOS {
    ...
    protected function saveMetadataToFS($metadata) {
        $file = "/tmp/virtualize/{$metadata->slug}/metadata.json" ;
        $string = json_encode($metadata) ;
        file_put_contents($file, $string) ;
    }

    protected function createTempDirectory($metadata) {
        $boxdir = BASE_TEMP_DIR.'virtualize'.DS . $metadata->slug ;
    ...
    protected function exportOVA($vmName, $metadata) {
        ...
        $command = VBOXMGCOMM." export {$vmName} --output=/tmp/virtualize/{$metadata->slug}/box.ova" ;
        self::executeAndOutput($command);
    ...
    protected function createPackage($target, $metadata) {
        ...
        $command = "tar -cvf $target$metadata->slug.box -C /tmp/virtualize/{$metadata->slug} . " ;
        self::executeAndOutput($command);
