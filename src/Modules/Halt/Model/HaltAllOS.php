...
class HaltAllOS extends BaseFunctionModel {
    ...
    public function haltNow() {
        ...
        $logging->log("Waiting at least {$this->virtualizerfile->config["vm"]["graceful_halt_timeout"]} seconds for machine to power off...") ;
        $this->provider->haltSoft($this->virtualizerfile->config["vm"]["name"]);
        if ($this->waitForStatus("powered off", $this->virtualizerfile->config["vm"]["graceful_halt_timeout"], "3")==true) {
            ...
            $sshParams["ssh-data"] = "echo {$this->virtualizerfile->config["ssh"]["password"]} | sudo -S shutdown now\n";
            ...
            $logging->log("Waiting at least {$this->virtualizerfile->config["vm"]["ssh_halt_timeout"]} seconds for machine to power off...") ;

            if ($this->waitForStatus("powered off", $this->virtualizerfile->config["vm"]["ssh_halt_timeout"], "3")==true) {
        ...
        if (isset($this->params["fail-hard"])) {
            ...
            $logging->log($lmsg) ;
            $this->provider->haltHard($this->virtualizerfile->config["vm"]["name"]);
            return true ; }
        $lmsg = "Attempts to Halt this box by both Soft Power off and SSH Shutdown have failed. You may need to use ".
            "virtualizer halt hard. You can also use the parameter --fail-hard to do this automatically." ;
        $logging->log($lmsg) ;
        return false ;
    ...
    public function haltPause() {
        $this->loadFiles();
        $this->findProvider("BoxHalt");
        $this->provider->haltPause($this->virtualizerfile->config["vm"]["name"]);
    }
    ...
    protected function currentStateIsHaltable() {
        ...
        if ($this->provider->isVMInStatus($this->virtualizerfile->config["vm"]["name"], $haltables) == true) {
    ...
    protected function waitForStatus($statusRequested, $total_time, $interval) {
        for ($i=0; $i<$total_time; $i=$i+$interval) {
            if($this->provider->isVMInStatus($this->virtualizerfile->config["vm"]["name"], $statusRequested)) {
                return true ; }
            echo "." ;
            sleep($interval); }
        echo "\n" ;
