...
class OSProvisioner extends ProvisionDefaultLinux {
    ...
    public function getCleopatraInitSSHData($provisionFile) {
		$sshData = "" ;
        $sshData .= "echo ".$this->virtualizerfile->config["ssh"]["password"]." | sudo -S apt-get update -y\n" ;
        $sshData .= "echo ".$this->virtualizerfile->config["ssh"]["password"]." | sudo -S apt-get install -y php5 git\n" ;
        $sshData .= "echo ".$this->virtualizerfile->config["ssh"]["password"]." | sudo -S rm -rf cleopatra\n" ;
        $sshData .= "echo ".$this->virtualizerfile->config["ssh"]["password"]." | sudo -S git clone https://github.com/PharaohTools/cleopatra.git\n" ;
        $sshData .= "echo ".$this->virtualizerfile->config["ssh"]["password"]." | sudo -S php cleopatra/install-silent\n" ;
        return $sshData ;
    }

    public function getMountSharesSSHData($provisionFile) {
        $sshData = "" ;
        $sshData .= "echo {$this->virtualizerfile->config["ssh"]["password"]} "
    ...
    public function getStandardCleopatraSSHData($provisionFile, $params = array() ) {
        ...
        $sshData = <<<"SSHDATA"
echo {$this->virtualizerfile->config["ssh"]["password"]} | sudo -S cleopatra auto x --af={$provisionFile}{$paramString}
SSHDATA;
        return $sshData ;
    }
    ...
    public function getStandardDapperstranoSSHData($provisionFile, $params = array() ) {
        ...
        foreach ($params as $paramKey => $paramValue) { $paramString .= " --$paramKey=$paramValue" ;}
        $sshData = <<<"SSHDATA"
echo {$this->virtualizerfile->config["ssh"]["password"]} | sudo -S dapperstrano auto x --af={$provisionFile}{$paramString}
SSHDATA;
        return $sshData ;
    }
    ...
    public function getStandardShellSSHData($provisionFile) {
        $sshData = <<<"SSHDATA"
echo {$this->virtualizerfile->config["ssh"]["password"]} | sudo -S sh $provisionFile
SSHDATA;
        return $sshData ;
    }
