...
class ProvisionDefaultLinux extends Base {
    ...
    public function provisionHook($hook, $type) {
        ...
        $logging->log("Provisioning from Phlagrantfile settings if available for $hook $type") ;
        $provisionOuts = $this->provisionPhlagrantfile($hook, $type) ;
    ...
    protected function provisionPhlagrantfile($hook, $type) {
        $provisionOuts = array() ;
        if (isset($this->phlagrantfile->config["vm"]["provision_{$hook}_{$type}"]) &&
            count($this->phlagrantfile->config["vm"]["provision_{$hook}_{$type}"])>0){
            foreach ($this->phlagrantfile->config["vm"]["provision_{$hook}_{$type}"] as $provisionerSettings) {
                $provisionOuts[] = $this->doSingleProvision($provisionerSettings) ; } }
        return $provisionOuts ;
    }
        ...
        foreach ($provisioners as $provisioner) {
            ...
            foreach ($tools as $tool) {
                ...
                foreach ($targets as $target) {
                    $dir = getcwd().DS."build".DS."config".DS."phlagrant".DS."hooks".DS."$provisioner".DS.
